...
function App() {
  ...
  const handleStart = (e) => {
    ...
    const distance = Math.sqrt(
      Math.pow(clientX - nockCenterX, 2) + Math.pow(clientY - nockCenterY, 2)
    );

    if (distance <= 20) {
      // 20px radius around nock
      setIsDragging(true);
  ...
  const handleMove = (e) => {
    ...
    const distance = Math.sqrt(
      pullDeltaX * pullDeltaX + pullDeltaY * pullDeltaY
    );
    // Increase sensitivity: use /1.8 instead of /4, and max stretch 180
    const stretch = Math.min(distance / 1.8, 180);
    ...
    setTimeout(() => {
      ...
      const aimLength = Math.sqrt(
        aimDeltaX * aimDeltaX + aimDeltaY * aimDeltaY
      );
      if (aimLength > 0) {
        ...
        const targetBox = getIntersectingTarget(
          arrowHeadPos.x,
          arrowHeadPos.y,
          laserEndX,
          laserEndY
    ...
    if (
      releaseZoneRef.current &&
      !releaseZoneRef.current.classList.contains("visible")
    ) {
      releaseZoneRef.current.classList.add("visible");
    }

    // Screen shake logic
    // REMOVE shake logic from here
  };
  ...
  const handleEnd = (e) => {
    ...
    if (inReleaseZone && activeTarget && arrowRef.current) {
      ...
      const targetPage = pageSectionsRef.current.find(
        (section) => section && section.id === targetName + "Page"
      );
      ...
      if (targetPage) {
        ...
        arrowRef.current.style.transform = `translate(-50%, -50%) translate(${deltaX}px, ${deltaY}px) rotate(${angle}deg) scale(0.3)`;

        setTimeout(() => {
          setArrowVisible(false); // Hide arrow after animation
          ...
          setTimeout(() => {
            setExplosion(false);
            setShake(false);
            setArrowVisible(true); // Reset arrow
            if (arrowRef.current) {
              arrowRef.current.style.transition = "";
  ...
  return (
    <div className={`hero-section${shake ? " shake" : ""}`}>
      {" "}
      {/* Add shake class */}
      {explosion && <div className="explosion-overlay"></div>}
      <NavigationTargets navBoxesRef={navBoxesRef} onStart={handleStart} />
      ...
      />
      <PageSections pageSectionsRef={pageSectionsRef} onBack={goBack} />
    </div>
  );
}
